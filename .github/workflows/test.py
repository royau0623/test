...
# Flask Routes
# --------------------------
@flask_app.route('/login', methods=['GET', 'POST'])  # Sensitive route requiring both methods
def login():
    """Authentication route handling PIN verification
    - GET: Displays PIN entry form
    - POST: Processes PIN submission for authentication
    """
...
def index():
    """Main application route for BitLocker key lookup
    - GET: Displays device search form (for authenticated users)
    - POST: Processes device search and displays results
    """
...
@flask_app.route('/qr/<int:qr_index>', methods=['GET'])  # Explicitly GET-only for read operations
